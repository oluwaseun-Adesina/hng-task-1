...
const mapExternalError = (externalApi, res) => res.status(502).json({ status: 'error', message: `${externalApi} returned an invalid response` });
function resolveCountry(lower) {
...
app.get('/api/profiles', async (req, res) => {
    ...
    try {
        ...
        const orderBy = { [resolvedSortBy]: order };
        const [total, profiles] = await Promise.all([
            prisma.profile.count({ where }),
            prisma.profile.findMany({
                where,
