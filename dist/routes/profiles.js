...
import { formatProfile, getAgeGroup, ISO_TO_NAME, buildWhere, parsePositiveInt, parseFloat01, buildPaginationLinks, VALID_AGE_GROUPS, VALID_GENDERS, VALID_SORT_BY, VALID_ORDERS, } from '../lib/profiles.js';
const router = Router();
// All profile routes require auth + API version header
router.use(requireAuth, requireApiVersion);
const mapExternalError = (api, res) => res.status(502).json({ status: 'error', message: `${api} returned an invalid response` });
...
router.get('/export', async (req, res) => {
    ...
    try {
        ...
        const rows = profiles.map(p => [p.id, p.name, p.gender, p.gender_probability, p.age, p.age_group, p.country_id, p.country_name, p.country_probability, p.created_at.toISOString()]
...
router.get('/', async (req, res) => {
    ...
    let min_age;
    let max_age;
    let min_gender_probability;
    let min_country_probability;
    ...
    try {
        ...
        const queryRecord = {};
        if (gender)
            queryRecord['gender'] = gender;
        if (age_group)
            queryRecord['age_group'] = age_group;
        if (country_id)
            queryRecord['country_id'] = country_id;
        if (sort_by)
            queryRecord['sort_by'] = sort_by;
        if (order !== 'desc')
            queryRecord['order'] = order;
